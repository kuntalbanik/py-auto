...
path_to_write = "part.xlsx"
path_to_read = "price.xlsx"
...
for i in range(1, row_r + 1):
    ...
    read_data[sheet_obj_r.cell(row=i, column=1).value] = sheet_obj_r.cell(row=i, column=3).value
...
# get all values column numner

for i in range(1, row_w + 1):
    cell_obj = sheet_obj_w.cell(row=i, column=1).value
    # print(cell_obj)
    if read_data.get(cell_obj) is not None:
        # print(cell_obj)
        # print(read_data[cell_obj])
        sheet_obj_w.cell(row=i, column=2).value = read_data[cell_obj]
